 ...
 * Route: /manager/sites/link-completed
 */
export function LinkCompleted() {
    ...
    const [statusFilter, setStatusFilter] = useState(location.pathname.includes('live-links') ? 'live' : '');
    ...
    const fetchData = useCallback(async (page = 1) => {
        ...
        try {
            ...
            const result = await managerAPI.getCompletedLinks(params);
            setData(result.data || []);
    ...
    useEffect(() => {
        if (location.pathname.includes('live-links')) {
    ...
    const handleCheckLink = async (groupIndex, linkIndex, link) => {
        ...
        try {
            ...
            const result = await managerAPI.checkLinkStatus({

 ...
 */
export function TasksTable({ tasks, onViewDetails, onApprove, onReject, loading }) {
    ...
    const filterOptions = [
        {
            ...
            options: [
                ...
                { value: 'PENDING_MANAGER_APPROVAL_1', label: 'Pending Approval (Initial)' },
                { value: 'ASSIGNED_TO_WRITER', label: 'Assigned to Writer' },
                { value: 'WRITING_IN_PROGRESS', label: 'Writing in Progress' },
                { value: 'PENDING_MANAGER_APPROVAL_2', label: 'Pending Approval (Content)' },
                { value: 'ASSIGNED_TO_BLOGGER', label: 'Assigned to Blogger' },
                { value: 'PUBLISHED_PENDING_VERIFICATION', label: 'Pending Verification' },
        ...
        },
        {
            ...
            options: [
                ...
                { value: 'writer', label: 'Assigned to Writer' },
                { value: 'blogger', label: 'Assigned to Blogger' }
            ]
        }
    ];

    // Pre-process data for custom filters (like assignment status)
    const processedTasks = React.useMemo(() => {
        if (!tasks) return [];
        return tasks.map(task => ({
            ...task,
            assignment_status: task.assigned_writer_id ? 'writer' : (task.assigned_blogger_id ? 'blogger' : 'unassigned')
        }));
    }, [tasks]);
    ...
    const {
        ...
        handleFilterChange,
        clearFilters
    } = useTableFilter(processedTasks, { current_status: 'all', assignment_status: 'all' });
                        ...
                        {filteredData.length === 0 && (
                            <tr>
                                <td colSpan={6} className="px-4 py-8 text-center" style={{ color: 'var(--text-muted)' }}>
                                    {tasks && tasks.length > 0 ? 'No matching tasks found' : 'No tasks found'}
                                </td>
                            </tr>
                        )}
                    </tbody>

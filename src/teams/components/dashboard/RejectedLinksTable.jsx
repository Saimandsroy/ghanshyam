...
export function RejectedLinksTable() {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const rejectedTasks = (response.tasks || []).filter(t => t.current_status === 'REJECTED').slice(0, 5);
          ...
          <tbody className="divide-y divide-border">
            {tasks.map(task => (
              <Fragment key={task.id}>
                <tr className="hover:bg-accent transition-colors duration-150 cursor-pointer" onClick={() => toggleRow(task.id)}>

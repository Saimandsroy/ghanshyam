...
export const DirectBloggerOrder = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setTimeout(() => navigate('/manager/pending/bloggers'), 1500);
        } catch (err) {
        ...
        <Layout>
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="premium-card p-6">
                    ...
                    {/* Website Table */}
                    <div className="premium-table-container max-h-[250px]">
                        <table className="premium-table">
                            ...
                            <tbody>
                                {filteredWebsites.map((website) => (
                                    <tr key={website.id}
                                        className={`cursor-pointer ${isSelected(website.id) ? 'bg-purple-500/5' : 'hover:bg-white/5'}`}
                                        onClick={() => handleToggleWebsite(website)}>
                                        <td className="text-center">
                                            <div className={`w-5 h-5 rounded border flex items-center justify-center ${isSelected(website.id) ? 'bg-purple-500 border-purple-500' : 'border-[var(--text-muted)]'}`}>
                                                {isSelected(website.id) && <CheckCircle className="h-3.5 w-3.5 text-white" />}
                                            </div>
                                        </td>
                                        <td className={isSelected(website.id) ? 'text-purple-400 font-medium' : ''}>{website.root_domain || website.domain_url}</td>
                                        <td>{website.dr || '-'}</td>
                    ...
                    <div className="flex items-center justify-between mt-4">
                        <span className="text-sm text-[var(--text-muted)]">{selectedWebsites.length} selected</span>

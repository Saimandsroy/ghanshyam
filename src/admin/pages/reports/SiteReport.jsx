...
export function SiteReport() {
  const [filters, setFilters] = useState({ root: '', start: '', end: '' });
  ...
  const groups = useMemo(() => {
    ...
    const map = new Map();
    for (const row of r) {
      const key = row.rootDomain;
      map.set(key, (map.get(key) || 0) + row.price);
    }
    const items = Array.from(map.entries()).map(([rootDomain, amount]) => ({ rootDomain, amount }));
    items.sort((a, b) => a.rootDomain.localeCompare(b.rootDomain));
    return items;
  }, [filters]);
    ...
    <div className="space-y-4">
      <div className="rounded-2xl p-4" style={{ backgroundColor: 'var(--card-background)', border: '1px solid var(--border)' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          <input value={filters.root} onChange={e => { setFilters({ ...filters, root: e.target.value }); setPage(1); }} placeholder="Root domain" className="rounded-xl px-3 py-2" style={{ backgroundColor: 'var(--background-dark)', border: '1px solid var(--border)', color: 'var(--text-primary)' }} />
        ...
        <div className="mt-3">
          <button onClick={() => { setFilters({ root: '', start: '', end: '' }); setPage(1); }} className="text-sm" style={{ color: 'var(--text-muted)' }}>Reset</button>
        </div>
      </div>
          ...
          <tbody>
            {pageGroups.map((g, i) => (
              <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                <td className="px-4 py-3" style={{ color: 'var(--text-secondary)' }}>{g.rootDomain}</td>

...
export function VendorReport() {
  const [filters, setFilters] = useState({ vendor: '', start: '', end: '' });
  ...
  const groups = useMemo(() => {
    ...
    if (filters.vendor) {
      ...
      r = r.filter(x => x.vendor.toLowerCase().includes(q));
    }
    ...
    for (const row of r) {
      ...
      map.set(key, (map.get(key) || 0) + row.price);
    }
    const items = Array.from(map.entries()).map(([vendor, amount]) => ({ vendor, amount }));
    ...
    <div className="space-y-4">
      <div className="rounded-2xl p-4" style={{ backgroundColor: 'var(--card-background)', border: '1px solid var(--border)' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          <input value={filters.vendor} onChange={e => { setFilters({ ...filters, vendor: e.target.value }); setPage(1); }} placeholder="Vendor" className="rounded-xl px-3 py-2" style={{ backgroundColor: 'var(--background-dark)', border: '1px solid var(--border)', color: 'var(--text-primary)' }} />
        ...
        <div className="mt-3">
          <button onClick={() => { setFilters({ vendor: '', start: '', end: '' }); setPage(1); }} className="text-sm" style={{ color: 'var(--text-muted)' }}>Reset</button>
        </div>
      </div>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
        <table className="w-full">
          <thead style={{ backgroundColor: 'var(--background-dark)' }}>
            <tr>
              <th className="text-left px-4 py-3 text-sm" style={{ color: 'var(--text-muted)' }}>Vendor</th>
          ...
          <tbody>
            {pageGroups.map((g, i) => (
              <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                <td className="px-4 py-3" style={{ color: 'var(--text-secondary)' }}>{g.vendor}</td>
